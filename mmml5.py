"""
5th iteration:
insert first few bars partway into the song.
lead note lengths
...
for x in range(songLength):
    for y in range(8):
        ...
        while y+int(16/noteLength)>8:
            noteLength = r.choice([4, 8, 16]) # no half notes, they clearly don't fit.
        ...
        # Now for the note length, which can be done by increasing y.
...
for x in range(songLength):
    ...
    if x%50==0 and x>0:
        for y in range(3):
            output+=song["voice2"][x]
        # Also change the instrument (again).
        output+="o4w"+str(r.randint(0,4)) # set the octave to 4 as well (gets screwy, but might work)
'''
