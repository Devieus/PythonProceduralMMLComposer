...
def algorithm(note):
    if note != "r":
        output=int(r.gauss(0,2.5))+notes.index(note)
    else:
        int(r.gauss(0,1.5))+notes.index(song["voice2"][-2][-1])
...
for x in range(songLength):
    for y in range(8):
        result+=(octaveSign+note) # Add the result to the dict. Maybe change length
        octaveSign=''
        dnote = int(r.gauss(0, 2.5)) + notes.index(note) if note != "r" else int(r.gauss(0, 1.5)) + notes.index(
            song["voice2"][-2][-1])
