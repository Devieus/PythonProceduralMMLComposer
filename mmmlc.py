"""
10th iteration:
Harmony=arps now, DWI.
Actual chord progression.
No tempo change for now, no intro repeats either.

"""
import random as r
octaveSign=''
title="Blank"
# Take a tempo between 10 and 13.
tempo=r.randint(10,14)
...
for x in range(songLength):
    ...
    if x % stretch == 0 and x > 0:
        # This is a cool idea still.
        riff=drumGen()
    '''# Add the first three bars again after every couple of bars.
    if x%stretch==0 and x>0:
...
for x in range(songLength):
    ...
    '''
    if x%stretch==0 and x>0: # Add the first three bars again after every 20 bars.
        output += "t" + str(temptempo)
        temptempo+=1
        stretch2+=1
        stretch+=stretch2
...
stretch2=5
temptempo=tempo+1
output+="\"\n\nCH3Verse_1.s = \"t"+str(tempo)+" \\8 w2 v35 o5 l16 " # Set up basic variables for the rhythm track.
w=3
for x in range(songLength):
    ...
    '''if x%stretch==0 and x>0: # Add the first three bars again after every 20 bars.
        ...
        output+="o"+song["bassOctaves"][x][0] if x >= songLength / 2 else "o"+str(int(song["bassOctaves"][x][0])+1) # set the octave back.'''
...
for x in range(songLength):
   # Add the track to the song.
    ...
    # Add the first three bars again after every 10 bars.
    '''if x %stretch == 0 and x>0:
        output+="t"+str(temptempo)
        for y in range(3):
