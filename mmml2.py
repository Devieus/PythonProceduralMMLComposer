...
for x in range(songLength*8):
    song["voice1"]+=r.choice(drums) # Add the result to the dict.
...
# ---------------------------------------------------------Output----------------------------------------------
output=";[ "+title+" ];\n\n"
output+="CH1Verse_1.s = \"t"+str(tempo)+" \\12 w5 v45 l8 " # Set up basic variables for the track.
output+=song["voice1"] # And then add it to the output
'''
...
'''
output+="\n\nCH4Verse_1.s = \"t"+str(tempo)+" \\2 w1 v45 o2 l2 " # Set up basic variables for the track.
for x in range(songLength):
